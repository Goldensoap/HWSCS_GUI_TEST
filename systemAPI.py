...
from sqliteAPI import SQL_option
...
def msg_pack(content:str)->str:
    ...
    crc = crc8_check(bytes.fromhex(content))
    msgstr = content+crc
...
def cmd_pack(content:str)->str:
    ...
    if len(label) == 1:
        label = '0'+label
    #计算身份信息、指令内容、标签的CRC值
    crc = crc_check(bytes.fromhex(identity+content+label))
    #组合报文，并按字节分割，进行透传准备
...
def update_data_tree(tree:dict,contain:dict)->bool:
...
def insert_data(data:list):
    ...
    for element in data:
        ...
        j = sql.is_table_exist(d["DEVICE"])
        if j == False:
            sql.create_new_table(d["DEVICE"])
